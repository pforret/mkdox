...
function Script:main() {
  ...
  new)
    ...
    [[ ! -f "$folder/docs/index.md" ]] && echo "# Mkdocs Material" >"$folder/docs/index.md"
    IO:progress "Start Docker container"
    docker run --rm -it --user "$(id -u)":"$(id -g)" -v "${PWD}":/docs "squidfunk/mkdocs-material" new "$folder"
    local YML="$folder/mkdocs.yml"
    if [[ $(< "$YML" wc -c) == 19 ]] ; then
      # overwrite mkdocs.yml
      IO:success "Overwrite $YML"
      echo "# generated by pforret/mkdox on $(date)" > "$YML"
      echo "site_name: Mkdocs Material" >> "$YML"
    fi
